...
public final class DOMPGPData extends DOMStructure implements PGPData {
    ...
    /**
     * Creates a <code>DOMPGPData</code> containing the specified key packet. and optional list of
     * external elements.
     *
     * @param keyPacket a PGP Key Material Packet as defined in section 5.5 of
     *                  <a href="http://www.ietf.org/rfc/rfc2440.txt"/>RFC 2440</a>. The array is
     *                  cloned to prevent subsequent modification.
     * @param other     a list of {@link XMLStructure}s representing elements from an external
     *                  namespace. The list is defensively copied to prevent subsequent
     *                  modification. May be <code>null</code> or empty.
     *
     * @throws NullPointerException     if <code>keyPacket</code> is <code>null</code>
     * @throws IllegalArgumentException if the key packet is not in the correct format
     * @throws ClassCastException       if <code>other</code> contains any entries that are not of
     ...
     *                  cloned to prevent subsequent modification.
     * @param keyPacket a PGP Key Material Packet as defined in section 5.5 of
     *                  <a href="http://www.ietf.org/rfc/rfc2440.txt"/>RFC 2440</a> (may be
     *                  <code>null</code>). The array is cloned to prevent subsequent modification.
     * @param other     a list of {@link XMLStructure}s representing elements from an external
     *                  namespace. The list is defensively copied to prevent subsequent
     *                  modification. May be <code>null</code> or empty.
     *
    ...
    public void marshal(Node parent, String dsPrefix, DOMCryptoContext context)
        ...
        if (keyId != null) {
            Element keyIdElem = DOMUtils.createElement(ownerDoc, "PGPKeyID", XMLSignature.XMLNS,
                    dsPrefix);
        ...
        if (keyPacket != null) {
            Element keyPktElem = DOMUtils.createElement(ownerDoc, "PGPKeyPacket",
        ...
        for (int i = 0, size = externalElements.size(); i < size; i++) {
            DOMUtils.appendChild(pdElem,
                    ((javax.xml.crypto.dom.DOMStructure) externalElements.get(i)).getNode());
        }
     ...
     *
     * This method only checks if the packet contains a valid tag. The contents of the packet should
