...
import it.eng.crypto.provider.MyDigesterOutputStream;
...
public final class DOMReference extends DOMStructure implements Reference, DOMURIReference {
    ...
    /**
     * Look up useC14N11 system property. If true, an explicit C14N11 transform will be added if
     * necessary when generating the signature. See section 3.1.1 of
     * http://www.w3.org/2007/xmlsec/Drafts/xmldsig-core/ for more info.
     *
     * If true, overrides the same property if set in the XMLSignContext.
     */
    private static boolean useC14N11 = ((Boolean) AccessController
     ...
     * @param transforms a list of {@link Transform}s. The list is defensively copied to protect
     ...
     * @throws ClassCastException   if any of the <code>transforms</code> are not of type
     *                              <code>Transform</code>
     */
    ...
    public DOMReference(String uri, String type, DigestMethod dm, List appliedTransforms,
	    Data result, List transforms, String id, Provider provider) {
	this(uri, type, dm, appliedTransforms, result, transforms, id, null, provider);
    }

    public DOMReference(String uri, String type, DigestMethod dm, List appliedTransforms,
	    Data result, List transforms, String id, byte[] digestValue, Provider provider) {
	if (dm == null) {
	...
	if (!allTransforms.isEmpty()) {
	    Element transformsElem = DOMUtils.createElement(ownerDoc, "Transforms",
		    XMLSignature.XMLNS, dsPrefix);
	    refElem.appendChild(transformsElem);
	    for (int i = 0, size = allTransforms.size(); i < size; i++) {
		DOMStructure transform = (DOMStructure) allTransforms.get(i);
	...
	Element digestValueElem = DOMUtils.createElement(ownerDoc, "DigestValue",
		XMLSignature.XMLNS, dsPrefix);
	if (digestValue != null) {
	...
	if (md == null) {
	    try {
		md = MessageDigest
			.getInstance(((DOMDigestMethod) digestMethod).getMessageDigestAlgorithm());
	    } catch (NoSuchAlgorithmException nsae) {
		throw new XMLSignatureException(nsae);
	    }
	}
	md.reset();
	MyDigesterOutputStream dos;
	...
	if (cache != null && cache.booleanValue() == true) {
	    ...
	    dos = new MyDigesterOutputStream(md, true);
	} else {
	    dos = new MyDigesterOutputStream(md);
	}
	...
	try {
		...
		if (context instanceof XMLSignContext) {
		    if (!c14n11) {
			Boolean prop = (Boolean) context
				.getProperty("org.apache.xml.security.useC14N11");
		...
		if (context instanceof XMLSignContext && c14n11 && !xi.hasUnprocessedInput()
			&& !xi.isOutputStreamSet()) {
		    DOMTransform t = new DOMTransform(TransformService.getInstance(c14nalg, "DOM"));
		    Element transformsElem = null;
		    String dsPrefix = DOMUtils.getSignaturePrefix(context);
		    if (allTransforms.isEmpty()) {
			transformsElem = DOMUtils.createElement(refElem.getOwnerDocument(),
	...
	if (dereferencedData instanceof ApacheData) {
		...
		try {
		    return new OctetStreamData(xsi.getUnprocessedInput(), xsi.getSourceURI(),
			    xsi.getMIMEType());
