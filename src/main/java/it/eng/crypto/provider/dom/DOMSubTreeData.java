...
/**
 * This is a subtype of NodeSetData that represents a dereferenced same-document URI as the root of
 * a subdocument. The main reason is for efficiency and performance, as some transforms can operate
 * directly on the subdocument and there is no need to convert it first to an XPath node-set.
 */
public class DOMSubTreeData implements NodeSetData {
    ...
    /**
     * This is an Iterator that contains a backing node-set that is not populated until the caller
     * first attempts to advance the iterator.
     */
	 ...
	 *
	 * @param node the node (document or element) referenced by the URI fragment. If null,
	...
	/**
	 * Recursively traverses the subtree, and returns an XPath-equivalent node-set of all nodes
	 * traversed, excluding any comment nodes, if specified.
	 *
