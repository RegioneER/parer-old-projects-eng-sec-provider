...
public final class DOMXMLSignature extends DOMStructure implements XMLSignature {
     ...
     * @param objs             a list of <code>XMLObject</code>s or <code>null</code> if not
     *                         specified. The list is copied to protect against subsequent
     *                         modification.
     * @param id               an optional id (specify <code>null</code> to omit)
     * @param signatureValueId an optional id (specify <code>null</code> to omit)
     *
    ...
    public void marshal(Node parent, Node nextSibling, String dsPrefix, DOMCryptoContext context)
        ...
        sigElem = DOMUtils.createElement(ownerDoc, "Signature", XMLSignature.XMLNS, dsPrefix);

        // append xmlns attribute
        if (dsPrefix == null || dsPrefix.length() == 0) {
            sigElem.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns", XMLSignature.XMLNS);
        } else {
            sigElem.setAttributeNS("http://www.w3.org/2000/xmlns/", "xmlns:" + dsPrefix,
                    XMLSignature.XMLNS);
        }
    ...
    public void sign(XMLSignContext signContext) throws MarshalException, XMLSignatureException {
        ...
        try {
            ksr = signContext.getKeySelector().select(ki, KeySelector.Purpose.SIGN,
        ...
        byte[] val = null;
        try {
            val = ((DOMSignatureMethod) si.getSignatureMethod()).sign(signingKey,
                    (DOMSignedInfo) si, signContext);
        } catch (InvalidKeyException | SignatureException ike) {
    ...
    public boolean equals(Object o) {
        ...
        boolean keyInfoEqual = (ki == null ? osig.getKeyInfo() == null
                : ki.equals(osig.getKeyInfo()));

        return (idEqual && keyInfoEqual && sv.equals(osig.getSignatureValue())
                && si.equals(osig.getSignedInfo()) && objects.equals(osig.getObjects()));
    }
    ...
    public class DOMSignatureValue extends DOMStructure implements SignatureValue {
        ...
        public boolean validate(XMLValidateContext validateContext) throws XMLSignatureException {
            ...
            try {
                ksResult = validateContext.getKeySelector().select(ki, KeySelector.Purpose.VERIFY,
                        sm, validateContext);
            ...
            try {
                validationStatus = ((DOMSignatureMethod) sm).verify(validationKey,
                        (DOMSignedInfo) si, value, validateContext);
            } catch (Exception e) {
        ...
        public boolean equals(Object o) {
            ...
            boolean idEqual = (id == null ? osv.getId() == null : id.equals(osv.getId()));
        ...
        public void marshal(Node parent, String dsPrefix, DOMCryptoContext context)
            ...
            sigValueElem = DOMUtils.createElement(ownerDoc, "SignatureValue", XMLSignature.XMLNS,
                    dsPrefix);
            if (valueBase64 != null) {
