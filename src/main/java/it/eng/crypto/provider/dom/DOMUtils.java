...
public class DOMUtils {
    ...
    /**
     * Sets an element's attribute (using DOM level 2) with the specified value and namespace
     * prefix.
     *
     ...
     * AND registers the ID value with the specified element. This is for resolving same-document ID
     * references.
     *
     * @param elem  the element to set the attribute on
     * @param name  the name of the attribute
	...
	if (attr != null) {
	    IdResolver.registerElementById(elem, attr);
	}
    }
    ...
    /**
     * Returns the attribute value for the attribute with the specified name. Returns null if there
     * is no such attribute, or the empty string if the attribute value is empty.
     *
     * <p>
     * This works around a limitation of the DOM <code>Element.getAttributeNode</code> method, which
     * does not distinguish between an unspecified attribute and an attribute with a value of "" (it
    ...
    /**
     * Checks if child element has same owner document before appending to the parent, and imports
     * it to the parent's document if necessary.
     */
